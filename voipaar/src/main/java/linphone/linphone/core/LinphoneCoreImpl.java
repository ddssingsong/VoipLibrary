...
import linphone.linphone.mediastream.Factory;
...
class LinphoneCoreImpl implements LinphoneCore {
    ...
    //dds_modify
    private native Object inviteAddressWithParams(long nativePtrLc, long to, long nativePtrParam, String masterKey);
    ...
    public synchronized LinphoneCall inviteAddressWithParams(LinphoneAddress to, LinphoneCallParams params, String masterKey) throws LinphoneCoreException {
        long ptrDestination = ((LinphoneAddressImpl) to).nativePtr;
        long ptrParams = ((LinphoneCallParamsImpl) params).nativePtr;

        LinphoneCall call = (LinphoneCall) inviteAddressWithParams(nativePtr, ptrDestination, ptrParams, masterKey);
        if (call != null) {
            return call;
        } else {
    ...
    private native void acceptCallWithParams(long nativePtr, long aCall,
                                             long params, String masterKey);

    @Override
    public synchronized void acceptCallWithParams(LinphoneCall aCall,
                                                  LinphoneCallParams params, String masterKey) throws LinphoneCoreException {
        acceptCallWithParams(nativePtr, getCallPtr(aCall), getCallParamsPtr(params), masterKey);
    }
    ...
    public Factory getMSFactory() {
        return (Factory) getMSFactory(nativePtr);
    }
